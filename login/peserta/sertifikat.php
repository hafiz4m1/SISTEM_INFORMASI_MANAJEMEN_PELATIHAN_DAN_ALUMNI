...
$data = mysqli_query($koneksi, "
    SELECT pp.*, p.nama_pelatihan, p.tanggal_mulai, p.tanggal_selesai, u.name as nama_instruktur
...
<?php $count=0; while ($row = mysqli_fetch_assoc($data)): $count++; ?>
  <div class="col-md-6 col-lg-4">
    <div class="card h-100 text-center p-4">
      ...
      <small class="text-muted mb-3 d-block"><?= date('d M Y', strtotime($row['tanggal_mulai'])) ?> - <?= date('d M Y', strtotime($row['tanggal_selesai'])) ?></small>
      <div class="mb-2" style="font-size:13px"><span class="text-muted">Instruktur:</span> <?= htmlspecialchars($row['nama_instruktur']) ?></div>
      ...
      <?php else: ?>
        <button class="btn btn-outline-secondary btn-sm" disabled>
          <i class="bi bi-hourglass me-1"></i> Belum Tersedia
        </button>
      <?php endif; ?>
    </div>
  </div>
