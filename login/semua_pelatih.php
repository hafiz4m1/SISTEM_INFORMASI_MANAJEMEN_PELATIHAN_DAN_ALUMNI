...
$instruktur = mysqli_query($koneksi,
    "SELECT i.id, u.name, COALESCE(i.bidang_keahlian, i.pendidikan, u.email, '-') AS keahlian,
            (SELECT COUNT(*) FROM pelatihan p WHERE p.instruktur_id=i.id) AS jml_pelatihan,
            (SELECT COUNT(*) FROM peserta_pelatihan pp JOIN pelatihan p ON pp.pelatihan_id=p.id WHERE p.instruktur_id=i.id) AS jml_peserta
     FROM instruktur i
...
<head>
  ...
  <title>Semua Pelatih | BPPMDDTT</title>
...
<nav class="navbar navbar-expand-lg sticky-top">
  <div class="container">
    <a class="navbar-brand" href="Infoalumni_pelatih.php">BPPMDDTT | Semua Pelatih</a>
...
<main class="py-5">
  <div class="container">
    <div class="text-center mb-5">
      ...
      <h1 class="section-title">Semua Pelatih</h1>
      <p class="section-sub mx-auto" style="max-width:680px;">Menampilkan semua pelatih beserta bidang keahlian, jumlah pelatihan, dan jumlah peserta.</p>
    </div>
    <div class="card mb-4">
      <div class="card-body p-3">
        <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between">
          <div class="fw-semibold">Total Pelatih: <?= mysqli_num_rows($instruktur) ?></div>
    ...
    <div class="table-responsive">
      <table class="table table-hover align-middle">
        ...
        <tbody>
          <?php $no = 1; while ($row = mysqli_fetch_assoc($instruktur)): ?>
          <tr>
            <td><?= $no++ ?></td>
            <td><?= htmlspecialchars($row['name']) ?></td>
            <td><?= htmlspecialchars($row['keahlian']) ?></td>
            <td><span class="badge bg-primary"><?= $row['jml_pelatihan'] ?></span></td>
            <td><span class="badge bg-success"><?= $row['jml_peserta'] ?></span></td>
          </tr>
          <?php endwhile; ?>
        </tbody>
      </table>
